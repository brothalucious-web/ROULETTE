...
public class RouletteScreen extends AbstractContainerScreen<RouletteMenu> {
	...
	static final int[] TIER_TEXT_COLORS = {0xFF404040, 0xFF2E7D32, 0xFF1B7F8C, 0xFF8E24AA, 0xFFB26A00};
	...
	protected void init() {
		...
		this.tabs = GameTabs.create(GameTabs.ROULETTE);
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
		...
		if (spinning && this.lastPocket != -1 && pocket != this.lastPocket) {
			ClientSounds.play("block.note_block.hat", 1.6F, 0.5F);
		}
		...
		if (outcome != this.lastOutcome) {
			switch (outcome) {
				case RouletteMenu.OUTCOME_LOSE -> ClientSounds.play("block.note_block.bass", 0.6F, 1.0F);
				case RouletteMenu.OUTCOME_WIN -> ClientSounds.play("entity.player.levelup", 1.0F, 1.0F);
				case RouletteMenu.OUTCOME_JACKPOT -> ClientSounds.play("ui.toast.challenge_complete", 1.0F, 1.0F);
				default -> { }
			}
	...
	public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
		...
		drawBall(graphics, bx, by);
	}

	static void drawBall(GuiGraphicsExtractor graphics, int bx, int by) {
