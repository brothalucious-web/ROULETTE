...
/** Wheel layout, odds and prize rules shared by the games, the server and the client screens. */
public final class RouletteRules {
	...
	public static int prizeTier(int betTier, int color) {
		return prizeTierForJump(betTier, color == GREEN ? 2 : 1);
	}

	/** Prize tier that is {@code jump} tiers above the bet, capped at Legendary. */
	public static int prizeTierForJump(int betTier, int jump) {
		return Math.max(1, Math.min(4, betTier + jump));
	}
